...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
  ...
  if (request.action === 'START_APPLYING') {
    console.log('Starting auto-apply process with settings:', request.settings);
    if (request.settings) {
      settings = { 
...
async function navigateToJobSearch() {
  ...
  const params = new URLSearchParams({
    ...
    f_WT: settings.jobType === 'full-time' ? '1' :
          settings.jobType === 'part-time' ? '2' :
          settings.jobType === 'contract' ? '3' :
          settings.jobType === 'temporary' ? '4' :
          settings.jobType === 'internship' ? '5' : '1',
    f_E: '4', 
    f_LF: 'f_AL', 
    sortBy: 'R' 
  });
...
async function startAutoApply() {
  ...
  while (isRunning) {
    ...
    for (let card of jobCards) {
      ...
      const jobCompany = card.querySelector('.job-card-container__company-name')?.textContent?.trim() || '';
      
      // Check if job title matches (case insensitive)
      if (!jobTitle.toLowerCase().includes(settings.jobTitle.toLowerCase())) {
        console.log('Skipping job with non-matching title:', jobTitle);
        continue;
      }

      console.log('Processing Easy Apply job:', { title: jobTitle, company: jobCompany });
      ...
      if (easyApplyButton) {
        ...
        if (applied) {
          jobsApplied++;
          updateJobCount();
          console.log('Successfully applied to job. Total jobs applied:', jobsApplied);
        } else {
